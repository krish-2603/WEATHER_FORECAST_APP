...
SearchButton.addEventListener('click', (event)=>{
    ...
    if(SearchCity.value.match(CheckCity) && len.length != 0){
        ...
        ErrorMsg.innerHTML ="Kindly Enter Valid City Name !";
...
const GetWeather =  (city) =>{
    const url = `http://api.weatherapi.com/v1/forecast.json?key=378603cebc014f298d5164956232702&q=${city}&days=8&aqi=no&alerts=no`;
...
const ShowWeather = (WeatherData) =>{
    ...
    Day7ConditionBox.innerHTML = `<p>Condition<span>${WeatherData.forecast.forecastday[7].day.condition.text}</span> </p>
